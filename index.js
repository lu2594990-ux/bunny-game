jQuery(() => {
    ...
    getContainer().append(
        ...
        '<div class="inline-drawer-content">' +
            ...
            '<div style="padding:3px 0;"><label style="font-size:12px;color:#888;">API地址（填到 /v1 即可）</label>' +
            ...
            '<button id="bny-save" style="margin-top:6px;padding:6px 20px;border-radius:8px;border:none;background:linear-gradient(135deg,#ff6b9d,#c44569);color:white;font-size:12px;cursor:pointer;width:100%;">💾 保存所有设置</button>' +'<div id="bny-status" style="padding:5px 0;font-size:12px;color:#888;">Bunny is hidden</div>' +
        '</div></div>'
    );
    ...
    $("#bny-test-api").on("click", function () {
        ...
        fetch(fullUrl, {
            ...
            r.html('<span style="color:#27ae60;">✅ 连接成功！回复: ' + reply + '</span>');saveS(true);
        }).catch(function (err) {
    ...
    styleEl.textContent = [
        ...
        ".pnl{position:fixed;width:90vw;max-width:400px;height:70vh;max-height:600px;background:#fffafc;border-radius:16px;box-shadow:0 10px 40px rgba(0,0,0,.18);display:none;flex-direction:column;overflow:hidden;pointer-events:auto;border:1px solid #fde2e8;}",
        ...
        ".pv{background:#f9f0f3;border:1px solid #fde2e8;border-radius:10px;padding:10px;font-size:12px;color:#666;min-height:40px;max-height:none;overflow-y:auto;line-height:1.6;white-space:pre-wrap;word-break:break-word;}",
        ...
        ".rs{display:none;}",
        ".rb{background:#f0faf0;border:1px solid #d0e8d0;border-radius:10px;padding:10px;font-size:12px;color:#333;min-height:40px;max-height:none;overflow-y:auto;line-height:1.6;white-space:pre-wrap;word-break:break-word;}",
        ...
        ".ed{background:#fff5f5;border:1px solid #fdd;border-radius:8px;padding:8px;font-size:10px;color:#c0392b;max-height:80px;overflow-y:auto;word-break:break-all;margin-top:4px;display:none;}"].join("\n");
    ...
    function writeMes(index, text) {
        try {
            ...
            if (d) {
                if (typeof messageFormatting === "function") d.innerHTML = messageFormatting(text, ctx.name2, false, false, index);
                else d.innerHTML = text.replace(/\n/g, "<br>");
            }
            ctx.saveChat();
        } catch (ex) { console.error("[Bunny]", ex); }
    }
    ...
    grabBtnEl.addEventListener("click", function (e) {
        ...
        origFull = msg.text; modContent = null; modFull = null; showingMod = true;
        ...
        setAB(false); stEl.textContent = ""; stEl.className = "st"; edEl.style.display = "none"; rsEl.style.display = "none"; swRow.style.display = "none";});
    ...
    function applyResult(newContent) {
        ...
        gFull = modFull; gContent = newContent; showingMod = true;
    ...
    swBtns.forEach(function (btn) {
        btn.addEventListener("click", function (e) {
            ...
            if (v === "orig" && origFull && !showingMod) return;
            if (v === "mod" && modFull && showingMod) return;
            swBtns.forEach(function (b) { b.classList.remove("active"); });
            this.classList.add("active");
            if (v === "orig") {
                writeMes(gIndex, origFull);
                gFull = origFull; gContent = origContent; showingMod = false;
                pvEl.textContent = origContent;
                tiEl.textContent = "≈ " + roughTk(origContent) + " tokens";
                stEl.textContent = "↩ 已切换到原文"; stEl.className = "st";} else {
                if (!modFull) return;
                writeMes(gIndex, modFull);
                gFull = modFull; gContent = modContent; showingMod = true;
                pvEl.textContent = modContent;
                tiEl.textContent = "≈ " + roughTk(modContent) + " tokens";
                stEl.textContent = "↪ 已切换到修改版"; stEl.className = "st ok";
            }
        });
    });
    ...
    function doPolish(mode) {
        var st = getS();
        if (!st.url || !st.key) { stEl.textContent = "⚠️ 请先填写API地址和Key！"; stEl.className = "st err"; return; }
        ...
        stEl.textContent = "🔄 " + label + "中..."; stEl.className = "st";
        callApi(prompt, gContent).then(function (r) {
            applyResult(r);
            stEl.textContent = "✅ " + label + "完成！"; stEl.className = "st ok"; setAB(false);
        }).catch(function (e) {
    ...
    fab.addEventListener("touchend", function (e) {
        e.preventDefault(); e.stopImmediatePropagation();
        varwd = dragging, wm = hasMoved; dragging = false; hasMoved = false;
