...
function callApi(sysPrompt, userText) {
    ...
    function doPolish(mode) {
        ...
        if (mode === "kill") {
            ...
            prompt = killPrefix + st.killPrompt + "\n\n同时也请按以下要求调整文风：\n\n" + stylePrefix + st.stylePrompt;
            label = "杀八股+文风";
        }

        var killWords = (mode === "kill" || mode === "both") ? parseKillWords(st.killPrompt) : [];
        var chunks = splitChunks(gContent);
            ...
            callApi(prompt, chunks[idx]).then(function (r) {
                if (killWords.length > 0) {
                    ...
                    if (residual.length > 0) {
                        ...
                        var fixPrompt = "以下文本仍然包含这些禁词：" + residual.join("、") + "\n请将它们替换为更自然的中文the表达。其余文字保持完全不变。直接输出修改后的全文，不要任何解释。";
                ...
                if (msg === "Load failed") {
                    msg = "Load failed（Safari网络错误）第" + (idx + 1) + "段 -请求地址: " + buildUrl(getS().url);
                }
    ...
    function openP(text) {
        ...
        if (!text && !or hasSearch) tipEl.style.display = "flex";
        posPanel();
